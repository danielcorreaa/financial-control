      ...
      {loading ? (
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {months
            ...
            return (
              <div key={m.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
                {/* Colored header */}
                <div className={`bg-gradient-to-r ${MONTH_GRADIENTS[m.month]} p-5 text-white`}>
                  ...
                  <p className="text-2xl font-bold mt-3">{formatCurrency(balance)}</p>
                  <p className="text-white/60 text-xs">saldo do mês</p>
                </div>
                ...
                <div className="p-5 space-y-2.5 text-sm">
                  ...
                  <Link to={`/months/${m.id}`}
                    className="flex items-center justify-center gap-2 w-full mt-2 py-2.5 rounded-xl
                      bg-slate-50 hover:bg-violet-50 text-gray-600 hover:text-violet-700
                      font-semibold text-sm transition-colors border border-gray-100 hover:border-violet-200">
